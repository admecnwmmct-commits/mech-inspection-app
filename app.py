...
def generate_pdf(header, responses):
    ...
    for section in SECTIONS:
        ...
        for sub in section['subsections']:
            ...
            tbl_data = [["#", "Inspection Item", "Answer", "Remarks"]]
            ...
            col_w = [0.8*cm, 9.2*cm, 2*cm, 5.4*cm]
    ...
    sig_data = [
        ["Inspector's Signature", "Date & Time", "Checked by / Countersigned"],
        ["\n\n" + "_"*30, datetime.now().strftime("%d-%m-%Y  %H:%M"), "\n\n" + "_"*30],
    ]
...
def generate_docx(header, responses):
    ...
    for ci, hd in enumerate(["Inspector's Signature", "Date & Time", "Checked by / Countersigned"]):
        ...
        for sub in section['subsections']:
            for item in sub['items']:
                ...
                if resp.get('answer') == 'no':
                    nc_count += 1
                    remark = resp.get('remark', '')
                    remark_str = f" _(Remark: {remark})_" if remark else ""
                    lines.append(f"• {item['label'][:80]}{remark_str}")
    if nc_count == 0:
    ...
    for section in SECTIONS:
        for sub in section['subsections']:
            for item in sub['items']:
                resp = responses.get(item['id'], {})
                if resp.get('answer') == 'no':
                    nc_items.append({'label': item['label'], 'remark': resp.get('remark',''), 'section': section['title']})
